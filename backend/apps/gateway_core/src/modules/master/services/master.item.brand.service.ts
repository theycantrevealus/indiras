...
export class MasterItemBrandService {
  ...
  async add(
    ...
    data.code =
      data.code ??
      `${modCodes[this.constructor.name].defaultCode}-${new Date().getTime()}`
      ...
      .then(async (result) => {
        if (result) {
          ...
          return await this.add(
            {
              name: term.name,
              remark: 'Auto Generated Brand',
            },
            account
          ).then((addResult) => {
            return addResult.payload as IMasterItemBrand
          })
        }
      })
  }
